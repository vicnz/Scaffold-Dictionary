...
def App(text, currentIndex):
    ...
    try:
        ...
        rowData["title"] = encodeAccentLetters(title.text) # Title
        rowData["origin"] = encodeAccentLetters(wordOrigin) # Word Origin
        ...
        for groupItem in definitionGroups:
            ...
            definitionItem['type'] = encodeAccentLetters(wordType(defType.get_attribute_list('class')[-1]))
            
            # Definition's Description
            descriptions = groupItem.findChild(class_='gl')
            definitionItem['description'] = encodeAccentLetters(descriptions.text) if descriptions != None else definitionItem['type'].capitalize()

            # Word Variants
            ilokanoWord = groupItem.findChild(class_="iiGroup")
            variant = {} # Word Variants
            variant['ilokano'] = encodeAccentLetters(ilokanoWord.findChild(class_='d').text) if ilokanoWord != None else None
            tagalogWord = groupItem.findChild(class_="itGroup")
            variant['tagalog'] = encodeAccentLetters(tagalogWord.findChild(class_='d').text) if tagalogWord != None else None
            ivatanWord = groupItem.findChild(class_="ivGroup")
            variant['ivatan'] = encodeAccentLetters(ivatanWord.findChild(class_='d').text) if ivatanWord != None else None
            ...
            for sampleGroup in groupItem.find_all(class_="exGroup"):
                ...
                if(ivatanExample != None and englishTranslation != None):
                    sampleItem['sample'] = encodeAccentLetters(ivatanExample.findChild(class_='d').text)
                    sampleItem['translation'] = encodeAccentLetters(englishTranslation.findChild(class_='d').text)
            ...
            # Grammar
            grammar = encodeAccentLetters(groupItem.findChild(class_='eg').text) if groupItem.findChild(class_='eg') != None else None
            definitionItem['grammar'] = grammar

            # Cultural Meaning
            culturalNote = encodeAccentLetters(groupItem.findChild(class_='ec').text) if groupItem.findChild(class_='ec') != None else None             
            ...
            for semanticSample in semantics:
                semanticsSampleList.append(encodeAccentLetters(semanticSample.text))
            ...
            for morpho in morphophonemics:
                morphophonemicsList.append(encodeAccentLetters(morpho.text))
            ...
            for synonym in synonyms:
                if synonym.text != None:
                    synonymsList.append(encodeAccentLetters(synonym.text))
            ...
            for related in relatedWords:
                if(related.text != None):
                    relatedWordsList.append(encodeAccentLetters(related.text))
            ...
            for antonym in antonyms:
                if(antonym.text != None):
                    antonymsList.append(encodeAccentLetters(antonym.text))
            ...
            # Derivative Word
            derivatedDefinition = {
                'derivative': encodeAccentLetters(derivative.findChild(class_='d').text) if derivative != None else None,
                'derivative_def': encodeAccentLetters(derivative.find_next_sibling().text) if derivative != None else None
            }
            ...
            for saying in sayings:
                sayingItem = {
                    'saying': encodeAccentLetters(saying.text),
                    'saying_desc': encodeAccentLetters(saying.find_next_sibling().text) if saying.find_next_sibling() != None else None
                }
        ...
        logger.error(f"file: {__file__} -> {e} : {traceback.print_exc()}")
        raise DatabaseError("")
